...
export default function NonInvoiceIncomePage() {
  ...
  const handleDelete = async () => {
    ...
    try {
      await client.delete(`/api/non-invoice-incomes/${confirmDeleteId}`)
      await load()
    } catch (e) {
      console.error(e)
    } finally {
  ...
  const handleModalSubmit = async (values: any) => {
    try {
      if (modalMode === "add") {
        await client.post("/api/non-invoice-incomes", values)
      } else {
        await client.put(`/api/non-invoice-incomes/${editingRow?.id}`, values)
      }
      await load()
      return true
    } catch (e) {
