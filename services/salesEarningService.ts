...
export async function getMonthlySalesEarning(params: SalesEarningParams) {
  ...
  const raw   = result.totals[0] ?? {
    totalSales: 0, totalPurchase: 0, totalDiscount: 0,
    totalServiceCharge: 0, totalCollected: 0,
  }
  ...
  const items = docs.map((d: any) => {
    ...
    return {
      ...
      serviceCharge,
      earningAmount:    netTotal - totalCost,
  ...
  return {
    ...
    totals: {
      ...
      purchaseAmount: raw.totalPurchase,
      clientDiscount: raw.totalDiscount,
      serviceCharge:  raw.totalServiceCharge,
      earningAmount:  raw.totalSales - raw.totalPurchase,
